...
The claim/evidence/verdict triple above is the model. Two commands, shipped
in v0.0.4, let you inspect and act on the trust decision it produces for a
specific claim: [Evidence graph](./evidence-graph.md) shows the full typed
evidence graph a claim's finding was computed from, and
[Fusion & decision](./fusion-and-decision.md) shows the aggregated verdict
and an actionable `PROCEED`/`REVIEW`/`BLOCK` recommendation computed on top
of it.
